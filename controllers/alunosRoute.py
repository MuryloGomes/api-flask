...
from datetime import datetime, date  # Corrigido aqui: importação da classe 'date'
...
def update_aluno(id):
    try:
        ...
        # Atualiza os campos do aluno, se fornecidos na requisição
        if 'nome' in data and isinstance(data['nome'], str) and data['nome'].strip():
            aluno.nome = data['nome']

        if 'idade' in data and isinstance(data['idade'], int) and data['idade'] > 0:
        ...
        # Atualiza a média final do aluno
